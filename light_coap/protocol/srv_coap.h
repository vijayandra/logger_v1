...
typedef struct
{
    ...
    scoap_buffer_t buf;          /* Option value */
} scoap_option_t;

typedef struct
{
    scoap_header_t hdr;          /* Header of the packet */
    scoap_buffer_t tok;          /* Token value, size as specified by hdr.tkl */
    uint8_t numopts;            /* Number of options */
    scoap_option_t opts[MAXOPT]; /* Options of the packet. For possible entries see
                                 * http://tools.ietf.org/html/rfc7252#section-5.10 */
    scoap_buffer_t payload;      /* Payload carried by the packet */
} scoap_packet_t;
...
typedef int (*scoap_endpoint_func)(scoap_rw_buffer_t *scratch, const scoap_packet_t *inpkt, scoap_packet_t *outpkt, uint8_t id_hi, uint8_t id_lo);
#define MAX_SEGMENTS 3  // 2 = /foo/bar, 3 = /foo/bar/baz
typedef struct
{
    int count;
    const char *elems[MAX_SEGMENTS];
} scoap_endpoint_path_t;

typedef struct
{
    scoap_method_t method;               /* (i.e. POST, PUT or GET) */
    scoap_endpoint_func handler;         /* callback function which handles this
                                         * type of endpoint (and calls
                                         * scoap_make_response() at some point) */
    const scoap_endpoint_path_t *path;   /* path towards a resource (i.e. foo/bar/) */
